...
if (isset($_GET['event_id']) && !empty($_GET['event_id'])) {
    ...
    $searchResult = mysqli_query($conn, "SELECT * FROM user_reports WHERE id LIKE '$eventId%'");
    ...
    $totalEntries = mysqli_query($conn, "SELECT COUNT(*) FROM user_reports")->fetch_row()[0];
    $totalPages = ceil($totalEntries / $entriesPerPage);
}
...
$sqlQuery = "SELECT * FROM user_reports";
...
if (isset($_GET['event_id']) && !empty($_GET['event_id'])) {
    ...
    if (isset($_GET['sort']) && $_GET['sort'] === 'category') {
        $rowsResult = mysqli_query($conn, "SELECT * FROM user_reports WHERE id LIKE '$eventId%' ORDER BY category DESC LIMIT $offset, $entriesPerPage");
    } else {
        // Default sort order is by created_at
        $rowsResult = mysqli_query($conn, "SELECT * FROM user_reports WHERE id LIKE '$eventId%' ORDER BY created_at DESC LIMIT $offset, $entriesPerPage");
    }
} else {
    // Fetch all events
    if (isset($_GET['sort']) && $_GET['sort'] === 'category') {
        $rowsResult = mysqli_query($conn, "SELECT * FROM user_reports ORDER BY category DESC LIMIT $offset, $entriesPerPage");
    } else {
        // Default sort order is by created_at
        $rowsResult = mysqli_query($conn, "SELECT * FROM user_reports ORDER BY created_at DESC LIMIT $offset, $entriesPerPage");
    }
}
...
<style> 
  ...
  <div class="w3-bar w3-black w3-card navbar">
    <a href="indexuser.php" class="w3-bar-item w3-button w3-padding-large w3-black" class="page-link"><i class="fa fa-arrow-left"></i> Report Event</a>
  ...
  <!-- Navbar on small screens -->
    <div id="navDemo" class="w3-bar-block w3-white w3-hide w3-hide-large w3-hide-medium w3-large">
      <a href="datauser.php" class="w3-bar-item w3-button w3-padding-large">Database Page</a>
      </div>
    </div>
    ...
    <?php else: ?>
        <?php foreach ($rows as $row): ?>
            <tr>
                ...
                <td><a href="edituser.php?id=<?php echo $row['id']; ?>" class='edit-button'><i class='fas fa-edit'></i> Edit</a></td>
            </tr>
        <?php endforeach; ?>
    <?php endif; ?>
</table>
