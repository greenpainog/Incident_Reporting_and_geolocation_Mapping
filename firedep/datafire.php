...
if (isset($_GET['event_id']) && !empty($_GET['event_id'])) {
    ...
    $searchResult = mysqli_query($conn, "SELECT * FROM firetweets WHERE id LIKE '$eventId%'");
    ...
    $totalEntries = mysqli_query($conn, "SELECT COUNT(*) FROM firetweets")->fetch_row()[0];
    $totalPages = ceil($totalEntries / $entriesPerPage);
}
...
$sqlQuery = "SELECT * FROM firetweets";
...
if (isset($_GET['event_id']) && !empty($_GET['event_id'])) {
    ...
    if (isset($_GET['sort']) && $_GET['sort'] === 'category') {
        $rowsResult = mysqli_query($conn, "SELECT * FROM firetweets WHERE id LIKE '$eventId%' ORDER BY category DESC LIMIT $offset, $entriesPerPage");
    } else {
        // Default sort order is by created_at
        $rowsResult = mysqli_query($conn, "SELECT * FROM firetweets WHERE id LIKE '$eventId%' ORDER BY created_at DESC LIMIT $offset, $entriesPerPage");
    }
} else {
    // Fetch all events
    if (isset($_GET['sort']) && $_GET['sort'] === 'category') {
        $rowsResult = mysqli_query($conn, "SELECT * FROM firetweets ORDER BY category DESC LIMIT $offset, $entriesPerPage");
    } else {
        // Default sort order is by created_at
        $rowsResult = mysqli_query($conn, "SELECT * FROM firetweets ORDER BY created_at DESC LIMIT $offset, $entriesPerPage");
    }
}
...
<style> 
  ...
  <div class="w3-bar w3-black w3-card navbar">
    <a href="indexfire.php" class="w3-bar-item w3-button w3-padding-large w3-black" class="page-link"><i class="fa fa-arrow-left"></i> Report Event</a>
    <div class="w3-bar-item w3-padding-large w3-black navbar-item" >NATIONAL FIRE DEPARTMENT</div>
    <a href="mapfire.php" class="link-button"  class="page-link"><i class="fa fa-map-marker"></i> View event map</a>
  </div>
</div>
...
<div class="w3-container w3-center" style="margin-top:50px">
    <h2>Search Event</h2>
    <form action="datafire.php" method="GET">
    ...
    <table class="w3-table w3-bordered w3-hoverable">
        ...
        <?php if (isset($row)): ?>
            <tr>
                ...
                <td><a href="editfire.php?id=<?php echo $row['id']; ?>">Edit</a></td>
            </tr>
        <?php else: ?>
            <?php foreach ($rows as $row): ?>
                <tr>
                    ...
                    <td><a href="editfire.php?id=<?php echo $row['id']; ?>" class='edit-button'><i class='fas fa-edit'></i> Edit</a></td>
                </tr>
            <?php endforeach; ?>
        <?php endif; ?>
    </table>
