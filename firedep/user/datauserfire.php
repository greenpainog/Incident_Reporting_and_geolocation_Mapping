...
if (isset($_GET['event_id']) && !empty($_GET['event_id'])) {
    ...
    $searchResult = mysqli_query($conn, "SELECT * FROM userfire_reports WHERE id LIKE '$eventId%'");
    ...
    $totalEntries = mysqli_query($conn, "SELECT COUNT(*) FROM userfire_reports")->fetch_row()[0];
    $totalPages = ceil($totalEntries / $entriesPerPage);
}
...
$sqlQuery = "SELECT * FROM userfire_reports";
...
if (isset($_GET['event_id']) && !empty($_GET['event_id'])) {
    ...
    if (isset($_GET['sort']) && $_GET['sort'] === 'category') {
        $rowsResult = mysqli_query($conn, "SELECT * FROM userfire_reports WHERE id LIKE '$eventId%' ORDER BY category DESC LIMIT $offset, $entriesPerPage");
    } else {
        // Default sort order is by created_at
        $rowsResult = mysqli_query($conn, "SELECT * FROM userfire_reports WHERE id LIKE '$eventId%' ORDER BY created_at DESC LIMIT $offset, $entriesPerPage");
    }
} else {
    // Fetch all events
    if (isset($_GET['sort']) && $_GET['sort'] === 'category') {
        $rowsResult = mysqli_query($conn, "SELECT * FROM userfire_reports ORDER BY category DESC LIMIT $offset, $entriesPerPage");
    } else {
        // Default sort order is by created_at
        $rowsResult = mysqli_query($conn, "SELECT * FROM userfire_reports ORDER BY created_at DESC LIMIT $offset, $entriesPerPage");
    }
}
...
<style> 
        ...
        body {
            background-color: red; /* Set background color of the page */
  ...
  <div class="w3-bar w3-black w3-card navbar">
    <a href="indexuserfire.php" class="w3-bar-item w3-button w3-padding-large w3-black" class="page-link"><i class="fa fa-arrow-left"></i> Report Event</a>
    <div class="w3-bar-item w3-padding-large w3-black navbar-item" >NATIONAL FIRE DEPARTMENT</div>
    <a href="" class="link-button"  class="page-link"><i class="fa fa-map-marker"></i> View event map</a>
  </div>
</div>

  <!-- Navbar on small screens -->
    <div id="navDemo" class="w3-bar-block w3-white w3-hide w3-hide-large w3-hide-medium w3-large">
      <a href="datauserfire.php" class="w3-bar-item w3-button w3-padding-large">Database Page</a>
      </div>
    </div>
...
<div class="w3-container w3-center" style="margin-top:50px">
    <h2>Search Event</h2>
    <form action="datauserfire.php" method="GET">
...
<table class="w3-table w3-bordered w3-hoverable">
    ...
    <?php if (isset($row)): ?>
        <tr>
            ...
            <td><?php echo !empty($row["location"]) ? $row["location"] : "No Location"; ?></td>
                    ...
                    <?php if (!empty($row["attachments"])): ?>
                        <?php
                            ...
                            foreach ($attachments as $attachment):
                                ...
                                echo "<p>File Path: " . htmlspecialchars($attachment) . "</p>";
                                if (in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif'])):
                        ?>
                                    <img src="<?php echo htmlspecialchars($attachment); ?>" alt="Attachment" style="width: 200px; height: auto; margin: 5px;">
                        <?php
                                else:
                        ?>
                                    <a href="<?php echo htmlspecialchars($attachment); ?>" target="_blank">Download Attachment</a>
                        <?php
                                endif;
                            endforeach;
                        ?>
            ...
            <td><a href="edituserfire.php?id=<?php echo $row['id']; ?>">Edit</a></td>
        </tr>
    <?php else: ?>
        <?php foreach ($rows as $row): ?>
            <tr>
                    ...
                    <?php if (!empty($row["attachments"])): ?>
                        <?php
                            ...
                            foreach ($attachments as $attachment):
                                ...
                                if (in_array($fileExtension, ['jpg', 'jpeg', 'png', 'gif'])):
                        ?>
                                    <img src="<?php echo htmlspecialchars($attachment); ?>" alt="Attachment" style="width: 150px; height: auto; margin: 5px;">
                        <?php
                                else:
                        ?>
                                    <a href="<?php echo htmlspecialchars($attachment); ?>" target="_blank">Download Attachment</a>
                ...
                <td><a href="edituserfire.php?id=<?php echo $row['id']; ?>" class='edit-button'><i class='fas fa-edit'></i> Edit</a></td>
            </tr>
        <?php endforeach; ?>
    <?php endif; ?>
</table>
...
<script>
    function clearSearch() {
        ...
        window.location.href = 'datauser.php'; // Redirect to the original page
    }
</script>
